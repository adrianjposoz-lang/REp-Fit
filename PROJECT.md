...
- **v0.8.6 (2026-04-20)** — Food search + privacy cleanup before shipping public. **Food search**: parallel USDA + Open Food Facts text search so branded grocery items resolve (USDA is whole-food heavy; OFF is branded-heavy). Added Settings → Food search section with optional user-supplied USDA API key (free signup at api.data.gov raises the shared `DEMO_KEY` ~30 req/hr ceiling to 1,000/hr) and an Open Food Facts on/off toggle. New `lib/foodSearchConfig.js` stores settings in a separate `rep_fit:foodSearch` slot that export/import never touches; `lib/usda.js` now reads the key via `getUSDAKey()` (user key → `VITE_USDA_KEY` env → `DEMO_KEY`), and `lib/openfoodfacts.js` gained `searchOFF()` with internal caching. Food screen runs both in `Promise.allSettled`, merges + dedupes by brand+name, and shows partial results if one backend fails. **De-personalized Setup**: new-account form no longer pre-fills Adrian's name / 1600 kcal / 175 P / 60 F / 90 C / 223.1 lb / 197 lb. Name is required and empty by default; numeric fields show neutral placeholders (2000 kcal, 150 P, etc.) and save blank. Added `NEW_USER_DEFAULTS` constant in `lib/constants.js`; `Setup.jsx` and Settings → Add profile both use it instead of spreading `ADRIAN_DEFAULTS`. Existing profiles are untouched.
