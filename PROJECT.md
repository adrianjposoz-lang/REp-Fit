...
- **v0.8.3 (2026-04-20)** — Fix: new profiles now start on Day 1 of 90, not Day 3. `Setup.jsx` was spreading `ADRIAN_DEFAULTS` into every new profile, which hardcoded Adrian's original `startDate: '2026-04-17'` and `endDate: '2026-07-15'`. Setup now overrides those with `today` and `today + 89` so every fresh account sees a clean 90-day runway.
